...
main(int argc, char **argv)
{
	int i, j;
	Itemsets tmp1, tmp2;
	candidate_frequencies_prev =
		calloc(OTH_ITEMSET_ARRAY_MAX, sizeof(struct itemsetPtr));
	...
	for (i = 1; i < ITEMSET_MAX_SIZE; i++) {
		...
		if (i == 1) {
			...
			for (j = 0; j < OTH_ITEMSET_ARRAY_MAX; j++) {
				candidate_frequencies_cur[j].itemset_ptr =
					NULL;
			}

		} else if (i == 2) {
			candidate_frequencies_cur =
				calloc(OTH_ITEMSET_ARRAY_MAX,
				       sizeof(struct itemsetPtr));
			frequent_frequencies_cur =
				calloc(OTH_ITEMSET_ARRAY_MAX,
				       sizeof(struct itemsetPtr));
			for (j = 0; j < OTH_ITEMSET_ARRAY_MAX; j++) {
				candidate_frequencies_cur[j].itemset_ptr =
					NULL;
				frequent_frequencies_cur[j].itemset_ptr =
					NULL;
			}
		} else {
			//deleted code here...
		...
		scandb(i, candidate_frequencies1, frequent_frequencies1,
		       candidate_frequencies_cur, frequent_frequencies_cur,
